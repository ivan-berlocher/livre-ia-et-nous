...
**Mais comment le modèle "apprend"-il vraiment ?**

Il n'y a pas de mémoire au sens humain. Pas de base de données de faits. Le modèle est constitué de **milliards de nombres** appelés **poids** (ou paramètres).

GPT-4 aurait environ **1 700 milliards de poids**. Chaque poids est un nombre décimal (comme 0.0034 ou -2.718).

**L'entraînement ajuste ces poids :**

1. Au début, les poids sont aléatoires → le modèle dit n'importe quoi
2. On lui montre une phrase, il prédit mal → on ajuste légèrement les poids
3. On répète des milliards de fois → les poids convergent vers des valeurs utiles

**Où sont les "connaissances" ?**

Elles sont **distribuées** dans les poids. Il n'y a pas un poids qui dit "Paris = capitale de France". Cette information est encodée de façon diffuse dans des millions de connexions.

C'est comme votre cerveau : il n'y a pas un neurone qui contient "le visage de votre mère". Cette information est distribuée dans des millions de connexions synaptiques.

**Conséquence importante :**

On ne peut pas "ouvrir" le modèle pour voir ce qu'il sait. On ne peut pas corriger une erreur spécifique sans risquer d'en créer d'autres. C'est une **boîte noire** — même pour ses créateurs.
...
### 2.2.1 RLHF : Comment ChatGPT est devenu "poli"

L'entraînement sur Internet ne suffit pas. Un modèle entraîné uniquement sur du texte web serait :
- Parfois grossier (il a vu des insultes)
- Souvent hors sujet (il continue le texte au lieu de répondre)
- Potentiellement dangereux (il a vu des contenus illégaux)

**Comment transformer ce modèle brut en assistant utile ?**

C'est là qu'intervient le **RLHF** — Reinforcement Learning from Human Feedback (Apprentissage par renforcement à partir du feedback humain).

**Le processus en 3 étapes :**

**Étape 1 : Collecte de préférences humaines**
- Des humains (souvent des travailleurs au Kenya, aux Philippines, ou ailleurs) comparent des paires de réponses
- "Cette réponse est-elle meilleure que celle-là ?"
- Critères : utile, honnête, inoffensive

**Étape 2 : Entraînement d'un modèle de récompense**
- On entraîne un modèle à prédire quelle réponse les humains préféreraient
- Ce modèle devient un "juge automatique"

**Étape 3 : Optimisation du modèle principal**
- Le modèle de langage est ajusté pour maximiser le score du "juge"
- Il apprend à produire des réponses que les humains préfèrent

**Résultat :**

Le modèle devient :
- Plus utile (il répond aux questions au lieu de juste continuer le texte)
- Plus poli (il évite les insultes et le langage offensant)
- Plus prudent (il refuse certaines demandes dangereuses)

**Le prix à payer :**

- Des milliers d'heures de travail humain (souvent mal payé)
- Un alignement sur les préférences d'OpenAI, pas les vôtres
- Des "garde-fous" qui peuvent être trop restrictifs ou contournables

**Pourquoi c'est important pour vous :**

Quand ChatGPT refuse de répondre à une question, ce n'est pas qu'il "ne peut pas". C'est qu'il a été entraîné à refuser. Ces refus reflètent les choix d'OpenAI, pas une limite technique.
